...
class C2DInterface extends Component {
  ...
  componentDidMount() {
    this.timerID = setInterval(() => this.fetchC2DInfo(), 5000);
    this.fetchC2DInfo();
  }
  ...
  sellC2D = async () => {
    ...
    if (C2DtoSell > 0) {
      ...
      console.log(
        "have " + web3.utils.toWei(this.state.c2dBal, "ether") + " C2D"
      );
  ...
  submitFormSwapCLVtoC2D(e) {
    e.preventDefault();
    if (
      parseFloat(this.state.c2dUserAllowance) < parseFloat(this.state.CLVtoSell)
    ) {
      this.CLVapproveC2D();
    } else {
      console.log(
        "Going to swap CLV: " +
          this.state.CLVtoSell * this.state.CLVscalar +
          " with allowance " +
          this.state.c2dUserAllowance
      );
      this.buyC2D();
    }
  }
  ...
  render() {
    ...
    return (
      <div className="station">
        ...
        <p>Dividends: {this.state.c2dDividends} CLV</p>
        <Button onClick={this.reinvestC2D}>reinvest</Button>
        ...
        <Form className="form" onSubmit={(e) => this.submitFormSwapCLVtoC2D(e)}>
          <Col>
            <Form.Group>
              ...
              />
              <Button>
                {parseFloat(this.state.c2dUserAllowance) < parseFloat(CLVtoSell)
                  ? "Approve"
                  : "Swap"}
              </Button>
            </Form.Group>
